<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ChatCustomer.aspx.cs" Inherits="Chat_ChatCustomer" %>
...
<body onload="javascript:UpdateLocalMessage();" style="height: 100%">
    ...
    <script type="text/javascript">
        ...
        function pageLoad(sender, args) {
            ...
            try {
                ...
                if (type=="E")
                    setInterval('GetVisitorTime()', 90000);
        ...
        function UpdateLocalMessage(result) {
            if (chatsession == true && result == true) {
                CloseWindow();
                return;
            }
        ...
        function UpdateMessageSuccessCallBack(args) {
            ...
            for (var i = 0; i < chatdetails.rows.length; i++) {
                ...
                if (chatmsgid < item.id) {
                    ...
                    else if ((item.Type == "E") && userid != item.fid) {
                        chatsession = true;
                        alert("Customer Closed chat")
                        CloseWindow();
                    }
        ...
        $("#divdisplaytext").click(function () {
            alert('Hi I am bound to the body!');
        });

        $('#txtcontact').click(function (e) {
        ...
        function addcontact() {
            ...
            var cmbContact = $find("<%=cmbContact.ClientID %>");
            var invite = document.getElementById('btnsend');
            if (visble == true) {
                cmbContact.set_visible(false);
                //  cmbContact.style.visibility = 'hidden';
                invite.style.visibility = 'hidden';
                visble = false;
            }
            else {
                //cmbContact.style.visibility = '';
                cmbContact.set_visible(true);
                invite.style.visibility = '';
                visble = true;
            }
            //alert("Hello");
        }
        var destid = 0;
        function cmbContactSelectedIndex(sender, eventArgs) {
            try {
                var item = eventArgs.get_item();

                var invite = document.getElementById('btnsend');
                destid = item.get_value();
            }
            catch (e) {
       ...
       <%-- function sendgroupcontactrequst() {
            var cmbContact = $find("<%=cmbtemplate.ClientID %>");
            var invite = document.getElementById('btnsend');
            if (destid != 0) {
                cmbContact.set_visible(false);
                invite.style.visibility = 'hidden';
                Insertchatgrouprequest(destid, szchatsessionid)
            }
            return false;
        }--%>
        ...
        ///--
        //var datatemp = "";
        //function cmbtemplateSelectedIndex(sender, eventArgs) {
        //    try {
        //        var item = eventArgs.get_item();


        //        datatemp = item.get_value();
        //    }
        //    catch (e) {
        ...
        //    if (datatemp != 0) {
        //        document.getElementById('txtmsg').value = datatemp;
        ...
        function Insertchatgrouprequest(destinationid, sessionId) {
            var nUserId = '<%= sourcid %>';
            ChatWS.Save_new_Chat_Entry_group(nUserId, destinationid, szchatsessionid, onsucesschatgroup)
        }
    ...
    <form id="form1" runat="server" defaultbutton="btnSendMessage">
        ...
        <div class="main">
            <div style="height:25px;">
                <div style="display: block" id="divgroup">
                    <a style="width: 20px; cursor: pointer;" onclick="javascript:addcontact()">

                        <telerik:RadToolTip ID="RadToolTip" runat="server" Width="50" HideEvent="LeaveToolTip" ManualCloseButtonText="close"
                            TargetControlID="ldcustomer" Position="BottomRight" Skin="WebBlue" Animation="Resize">
                        </telerik:RadToolTip>
                        <img src="../Images/accounts.png" alt="" id="ldcustomer" runat="server" /></a>

                 <%--   <telerik:RadComboBox ID="cmbtemplate" runat="server" Height="100" Width="180px" OnClientSelectedIndexChanging="cmbtemplateSelectedIndex">
                    </telerik:RadComboBox>
                    <asp:Button ID="Button1" runat="server" Text="Add" OnClientClick="return gettemplate();" />--%>

                    <span style="display: none">
                        <telerik:RadComboBox ID="cmbContact" runat="server" Height="100" Width="180px" OnClientSelectedIndexChanging="cmbContactSelectedIndex"
                            EmptyMessage="" EnableLoadOnDemand="true" MinFilterLength="1" ShowMoreResultsBox="true"
                            EnableVirtualScrolling="true" Style="visibility: hidden">
                            <WebServiceSettings Method="GetContact" Path="ChatCustomer.aspx">
                            </WebServiceSettings>
                        </telerik:RadComboBox>
                        <asp:Button ID="btnsend" runat="server" Text="Add" Style="visibility: hidden" OnClientClick="return sendgroupcontactrequst();" />
                    </span>
                </div>
            </div>
            <asp:Label ID="lblmessage" runat="server" Height="16px"></asp:Label>
            <div id="divdisplaytext" style="min-height: 206px; width: 274px; word-wrap: break-word; font-size: 11px; overflow: auto; border: 1px solid rgba(230, 229, 229, 0.76); padding: 4px; position: relative">
            </div>
            <div style="clear: both;">
            </div>
            <span id="lblstatus" style="height: 15px; padding-left: 15px; text-align: center; font-size: 9px; color: Gray; font-weight: normal;"></span>
            <div style="clear: both;">
            </div>
            <div style="width: 274px; float: left;">
                <div style="width: 235px; float: left">
                    <asp:TextBox ID="txtmsg" runat="server" Style="resize: none; width: 100%" TextMode="MultiLine"
                        onkeyup="cleartext(event)" onkeydown="sendMessage(event);"></asp:TextBox>
                </div>
                <div style="width: 28px; float: right; bottom: 0;">
                    <asp:ImageButton ID="ImageButton1" runat="server" ToolTip="End Chat" ImageUrl="~/Images/close_chat.png" OnClientClick="startchat('E'); return false;" />
                    <asp:ImageButton ID="btnSendMessage" runat="server" ToolTip="Enter" ImageUrl="~/Images/key_enter.png" OnClientClick="startchat('M');return false;" />
                </div>
            </div>
